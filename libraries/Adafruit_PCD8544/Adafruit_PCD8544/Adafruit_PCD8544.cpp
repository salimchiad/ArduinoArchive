...
void Adafruit_PCD8544::init(uint8_t contrast) { // Albert compatibility with older version
  begin(contrast);
}

void Adafruit_PCD8544::begin(uint8_t contrast) { // Albert repeat begin() to overcome bad LCD contacts
  ...
  if (_rst > 0) {
    ...
    delayMicroseconds(1); // Albert required is just 100ns
  ...
  setContrast(contrast); // Albert avoid duplicate code
 ...
 // display(); // Albert because init() is repeated, display() takes too much time
}
...
void Adafruit_PCD8544::clear(void) { // Albert compatibility with older version
  clearDisplay();
}  
