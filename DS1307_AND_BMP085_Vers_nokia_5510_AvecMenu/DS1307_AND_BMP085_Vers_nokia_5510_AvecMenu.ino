...
void LcdInitialise(void)
{
  ...
  LcdWrite(LCD_CMD, 0xAB);  // Set LCD Vop (Contrast). //B1
...
void loop(void)
{
  ...
  pea = bmp.readPressure();
  if (pea < 100000) {
    sprintf (pef, "0%ld", pea);
  }
  else {
    sprintf (pef, "%ld", pea);
  }
  ...
  // Preparation a l'affichage d'acceuil (date heure teperature... )
  if (disp == 0) {
    sprintf (LigneA, "%s %s %s %d", Day[dayOfWeek], jourf, Mon[month], year);
    sprintf (LigneB, "  %s:%s:%s", hourf, minutef, secondf);
    sprintf (LigneC, "Temp %s,%s'C", tempef, tempedf);
    sprintf (LigneD, "Pe %s Pa", pef);
    sprintf (LigneE, "            ");
    sprintf (LigneF, "            ");
    if (com == 16712445 || com == 16732845) {
      disp = 10;
      com = 0;
    }
  }
  ...
  if (disp > 0 && disp < 15) {
    sprintf (LigneA, " *  MENU  * ");
    if (disp == 10) {
      sprintf (LigneB, "*Affichage  ");
    } else {
      sprintf (LigneB, " Affichage  ");
    }

    if (disp == 11) {
      sprintf (LigneC, "*ChronoTimer");
    } else {
      sprintf (LigneC, " ChronoTimer");
    }

    if (disp == 12) {
      sprintf (LigneD, "*Date/heure  ");
    } else {
      sprintf (LigneD, " Date/heure  ");
    }

    if (disp == 13) {
      sprintf (LigneE, "*Alarmes    ");
    } else {
      sprintf (LigneE, " Alarmes    ");
    }

    if (disp == 14)  {
      sprintf (LigneF, "*Tests      ");
    } else {
      sprintf (LigneF, " Tests      ");
    }
  ...
  if (disp > 19 && disp < 26) {
    sprintf (LigneA, "  AFFICHAGE ");
    if (disp == 20) {
      sprintf (LigneB, "*Contast %d ", contrast);
    } else {
      sprintf (LigneB, " Contast %d ", contrast);
    }

    if (disp == 21) {
      sprintf (LigneC, "*Facteur t ");
    } else {
      sprintf (LigneC, " Facteur t ");
    }

    if (disp == 22) {
      sprintf (LigneD, "*Luminosite");
    } else {
      sprintf (LigneD, " Luminosite");
    }

    if (disp == 23) {
      sprintf (LigneE, "*a voir     ");
    } else {
      sprintf (LigneE, " a voir     ");
    }

    if (disp == 24)  {
      sprintf (LigneF, "*a voir     ");
    } else {
      sprintf (LigneF, " a voir     ");
    }
    ...
    if (disp > 29 && disp < 32) {
    sprintf (LigneA, "Menu Chrono");
    if (disp == 30) {
      sprintf (LigneB, "*  Chrono   ");
    } else {
      sprintf (LigneB, "   Chrono   ");
    }

    if (disp == 31) {
      sprintf (LigneC, "*  TIMER    ");
    } else {
      sprintf (LigneC, "   TIMER    ");
    }
...
sprintf (LigneF, "            ");
  ...
  // Envoi des instructions a l'ecran
  gotoXY(0, 0);
  LcdString(LigneA);
  gotoXY(0, 1);
  LcdString(LigneB);
